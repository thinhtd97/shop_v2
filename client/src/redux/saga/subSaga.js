...
function* detailSub(action) {
    try {
        const { slug } = action;
        const { data } = yield call(() => axios.get(`${process.env.REACT_APP_API}/sub/${slug}`));
        yield put({ type: subConstants.DETAILS_SUB_CATEGORY_SUCCESS, payload: data });
    } catch (error) {
        yield put({ 
            type: subConstants.DETAILS_SUB_CATEGORY_FAILED, 
            payload: error.response && error.response.data.message ? 
            error.response.data.message : 
            error.message 
        });
...
function* updateSub(action) {
    try {
        const { slug, authToken, sub, parent, history } = action;
        const config = {
            headers: {
                authToken
            }
        }
        const { data } = yield call(() => axios.put(`${process.env.REACT_APP_API}/sub/${slug}`,{ name: sub, parent } ,config));
        yield put({ type: subConstants.UPDATE_SUB_CATEGORY_SUCCESS });
        notification['success']({
            description: `${data.name} is updated.`
        })
        history.push('/admin/sub/list');
    } catch (error) {
        yield put({ 
            type: subConstants.UPDATE_SUB_CATEGORY_FAILED, 
            payload: error.response && error.response.data.message ? 
            error.response.data.message : 
            error.message 
        });
...
export default function* subSaga() {
    ...
    yield takeEvery(subConstants.DETAILS_SUB_CATEGORY_REQUEST, detailSub);
    yield takeEvery(subConstants.UPDATE_SUB_CATEGORY_REQUEST, updateSub);
}